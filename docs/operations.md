...
`nexus-trace-profile-yard` is centered on observability work in Scala. The technical target is: Package a Scala local lab for profile analysis with windowed input fixtures, late-data behavior checks, and documented operating limits.
...
The score is deterministic. The current threshold is 180. Risk has a penalty of 6, latency has a penalty of 4, and weight contributes a bonus of 3.
